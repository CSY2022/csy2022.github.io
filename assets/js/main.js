...
function getBingImages(img) {
  ...
  const img_content = img[index];
  const url = "https://cn.bing.com" + img_content.url;
  panel.style.background = "url('" + url + "') center center no-repeat #666";
  panel.style.backgroundSize = "cover";
  sessionStorage.setItem(indexName, index);

  const copyrightLabel = document.querySelector("#background-copyright");
  copyrightLabel.innerHTML = `<a href="${img[index].copyrightLink}">${img[index].copyright}</a>`;
}
